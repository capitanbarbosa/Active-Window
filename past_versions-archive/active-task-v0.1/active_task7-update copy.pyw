...
def show_result(index):
    ...
    name_text = f"🪶 Log --> {name.strip()}"  # remove newline character
    input_box.insert('1.0', name_text)
    props_box.delete('1.0', 'end')
    note_text = f"📝 Note --> {note.strip()}"  # remove newline character
...
def toggle_pomodoro():
    global pomodoro_running, pomodoro_button
...
def round_button(widget):
    ...
    widget.config(bd=0, padx=10, pady=5, font=("Arial", 9), width=6, height=2)
...
plus_button.grid(row=0, column=0, padx=5, pady=0)

# Create update page button a1
a1_button = tk.Button(frame, text="💾", command=update_database)
round_button(a1_button)
a1_button.grid(row=1, column=0, padx=5, pady=0)

# Create toggle size button
current_size = "1050x20"
size_button = tk.Button(frame, text="◻️", command=toggle_size)
round_button(size_button)
size_button.grid(row=0, column=2, padx=5, pady=0)

# Create move/lock button
move_button = tk.Button(frame, text="move/lock", command=move_lock)
round_button(move_button)
move_button.grid(row=1, column=2, padx=5, pady=0)

# Create pomodoro button
pomodoro_button = tk.Button(frame, text="⏰", command=toggle_pomodoro)
round_button(pomodoro_button)
pomodoro_button.grid(row=0, column=1, padx=5, pady=0)
...
a2_button = tk.Button(frame, text="🪄 magick")
round_button(a2_button)
a2_button.grid(row=1, column=1, padx=5, pady=0)
